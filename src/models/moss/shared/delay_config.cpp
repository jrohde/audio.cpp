...
BackboneConfig parse_backbone_config(const json::Value & value) {
...
Config parse_config(const json::Value & root, const char * family) {
    const auto model_type = json::optional_string(root, "model_type", "");
    if (model_type != "moss_tts_delay") {
        throw std::runtime_error(
            std::string(family) + " config model_type mismatch: expected moss_tts_delay, got " + model_type);
    }
    Config config;
    ...
    if (config.audio_pad_code != config.audio_vocab_size) {
        throw std::runtime_error(
            std::string(family) + " expects audio_pad_code to be the code past audio_vocab_size");
    }
    return config;
}

Config parse_config(const assets::ResourceBundle & resources, const char * family) {
    return parse_config(resources.parse_json("config"), family);
}
