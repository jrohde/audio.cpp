...
std::vector<std::pair<std::string, std::string>> parse_session_options(int argc, char ** argv) {
    ...
    for (int i = 1; i + 1 < argc; ++i) {
        ...
        if (eq == std::string::npos || eq == 0) {
            throw std::runtime_error("invalid ControlFoley --session-option: " + option);
        }
...
engine::core::BackendType parse_backend(const std::string & value) {
    ...
    throw std::runtime_error("ControlFoley warmbench is CUDA-only");
}
...
engine::runtime::TaskRequest make_request(const engine::io::json::Value & object) {
    engine::runtime::TaskRequest request;
    request.text_input = engine::runtime::Transcript{optional_string(object, "prompt"), ""};
    set_optional_option(request, object, "duration_sec");
    set_optional_option(request, object, "cfg_strength");
    set_optional_option(request, object, "negative_prompt");
    set_optional_option(request, object, "negative_text");
    set_optional_option(request, object, "video");
    set_optional_option(request, object, "mask_away_clip");
    set_optional_option(request, object, "seed");
    set_mapped_optional_option(request, object, "duration", "duration_sec");
    set_mapped_optional_option(request, object, "num_steps", "num_inference_steps");
    set_mapped_optional_option(request, object, "num_inference_steps", "num_inference_steps");
    if (const auto audio = optional_string(object, "audio"); !audio.empty()) {
        request.audio_input = read_audio_buffer(audio);
    }
    return request;
}
...
std::vector<engine::runtime::TaskRequest> parse_requests(const std::string & request_sequence_json) {
    if (request_sequence_json.empty()) {
        throw std::runtime_error("ControlFoley warmbench requires --request-sequence-json");
    }
    ...
    if (requests.empty()) {
        throw std::runtime_error("ControlFoley warmbench request sequence is empty");
    }
    return requests;
}
...
engine::io::json::Value audio_summary_json(const engine::runtime::AudioBuffer & audio) {
    if (audio.samples.empty()) {
        throw std::runtime_error("ControlFoley warmbench received empty audio output");
    }
    double sum = 0.0;
    double abs_sum = 0.0;
    double sq_sum = 0.0;
...
engine::io::json::Value step_json(
    ...
    if (!result.audio_output.has_value()) {
        throw std::runtime_error("ControlFoley warmbench expected audio output");
    }
...
int main(int argc, char ** argv) {
    try {
        const std::filesystem::path model_path = arg_value(argc, argv, "--model", "models/ControlFoley-GGUF/controlfoley-large-44k-f32.gguf");
        ...
        const std::filesystem::path timing_path =
            arg_value(argc, argv, "--timing-file", "/tmp/controlfoley_warm_bench_timing.log");
        ...
        load_request.family_hint = "controlfoley";
        ...
        if (session == nullptr) {
            throw std::runtime_error("loaded ControlFoley session is not offline-capable");
        }
        ...
        for (size_t request_index = 0; request_index < requests.size(); ++request_index) {
            ...
            std::cout << "controlfoley.wall_ms=" << wall_ms << "\n";
        ...
        std::cerr << "controlfoley_warm_bench failed: " << ex.what() << "\n";
        return 1;
    }
}
