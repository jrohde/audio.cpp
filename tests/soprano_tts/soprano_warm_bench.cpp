...
std::string required_string(const engine::io::json::Value & object, const std::string & key) {
    ...
    if (value.empty()) {
        throw std::runtime_error("Soprano warmbench request missing field: " + key);
    }
    return value;
}
...
SopranoBenchRequest make_request(const engine::io::json::Value & object, const std::string & fallback_id) {
    SopranoBenchRequest out;
    ...
    set_optional_option(out.request, object, "top_p", "top_p");
    set_optional_option(out.request, object, "repetition_penalty", "repetition_penalty");
    set_optional_option(out.request, object, "eos_bias", "eos_bias");
    set_optional_option(out.request, object, "seed", "seed");
    return out;
}

std::vector<SopranoBenchRequest> parse_requests(const std::string & request_sequence_json) {
    if (request_sequence_json.empty()) {
        throw std::runtime_error("Soprano warmbench requires --request-sequence-json");
    }
    const auto root = engine::io::json::parse(request_sequence_json);
    std::vector<SopranoBenchRequest> requests;
    int index = 0;
    for (const auto & item : root.as_array()) {
        requests.push_back(make_request(item, "request_" + std::to_string(index++)));
    }
    if (requests.empty()) {
        throw std::runtime_error("Soprano warmbench request sequence is empty");
    }
    return requests;
}

// Parses the shared ``[section]`` case catalog (same file the Python reference
// warmbench consumes: tests/soprano_tts/soprano_warm_bench_cases.txt).
std::vector<SopranoBenchRequest> parse_cases(
    const std::filesystem::path & cases_path,
    const std::vector<std::string> & sections) {
    std::ifstream file(cases_path);
    if (!file) {
        throw std::runtime_error("Soprano warmbench cannot open cases file: " + cases_path.string());
    }
    std::vector<SopranoBenchRequest> requests;
    std::string section;
    std::string line;
    while (std::getline(file, line)) {
        ...
        SopranoBenchRequest request;
        request.id = section + "_" + std::to_string(
            std::count_if(requests.begin(), requests.end(),
                [&section](const SopranoBenchRequest & item) {
                    return item.id.rfind(section + "_", 0) == 0;
                }));
        request.request.text_input = engine::runtime::Transcript{line, "en"};
        requests.push_back(std::move(request));
    }
    if (requests.empty()) {
        throw std::runtime_error("Soprano warmbench cases file produced no requests");
    }
    return requests;
}

std::optional<SopranoBenchRequest> parse_warmup_request(const std::string & request_json) {
...
engine::io::json::Value audio_summary_json(const engine::runtime::AudioBuffer & audio) {
    if (audio.samples.empty()) {
        throw std::runtime_error("Soprano warmbench received empty audio output");
    }
    double sum = 0.0;
    double abs_sum = 0.0;
    double sq_sum = 0.0;
...
engine::io::json::Value step_json(
    ...
    const SopranoBenchRequest & request,
    int request_index,
    double wall_ms,
    const std::filesystem::path & audio_path) {
    if (!result.audio_output.has_value()) {
        throw std::runtime_error("Soprano warmbench expected audio output");
    }
...
#error "soprano_warm_bench requires the ENGINE_REPO_ROOT compile definition (provided by its CMake target)"
#endif
        const std::filesystem::path repo_root = ENGINE_REPO_ROOT;
        const std::filesystem::path model_path = engine::tools::arg_value(argc, argv, "--model", "models/soprano_pkg");
        ...
        const std::filesystem::path timing_path =
            engine::tools::arg_value(argc, argv, "--timing-file", "build/logs/warmbench/soprano_tts_timing.log");
        const auto warmup_request =
            parse_warmup_request(engine::tools::arg_value(argc, argv, "--warmup-request-json", ""));
        const auto request_sequence_json = engine::tools::arg_value(argc, argv, "--request-sequence-json", "");
        const std::filesystem::path cases_path = engine::tools::arg_value(
            argc, argv, "--cases", (repo_root / "tests/soprano_tts/soprano_warm_bench_cases.txt").string());
        const auto sections = engine::tools::split_csv(engine::tools::arg_value(argc, argv, "--sections", ""));
        ...
        load_request.family_hint = "soprano_tts";
        ...
        if (session == nullptr) {
            throw std::runtime_error("loaded Soprano session is not offline-capable");
        }

        const auto requests = request_sequence_json.empty()
            ? parse_cases(cases_path, sections)
            : parse_requests(request_sequence_json);
        ...
        for (size_t request_index = 0; request_index < requests.size(); ++request_index) {
            ...
            std::cout << "soprano.request[" << request_index << "].id=" << requests[request_index].id << "\n";
        ...
        const auto summary = engine::io::json::Value::make_object({
            {"family", engine::tools::string("soprano_tts")},
        ...
        std::cerr << "soprano_warm_bench failed: " << ex.what() << "\n";
        return 1;
    }
}
